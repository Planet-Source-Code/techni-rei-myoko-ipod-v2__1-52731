...
'this is ddirected at a specific user who enjoys stealing my code
...
Public Sub setkeycontents(Section As String, key As String, Value As String, Optional Index As Long = 0)
    ...
    If temp > 0 Then
        If keyexists(temp, key, Index) = True Then
...
End If
End Function
Public Sub CreateQualifiedSection(Section As String)
Dim tempstr() As String, count As Long, tempstr2 As String, temp As Long, temp2 As Long
tempstr = Split(Section, "\")
For count = 0 To UBound(tempstr)
    If count = 0 Then
        tempstr2 = tempstr(0)
        temp = handlerootsections(tempstr2)
        If temp = -1 Then
            newrootsection tempstr2
            temp = handlerootsections(tempstr2)
        End If
    Else
        tempstr2 = tempstr2 & "\" & tempstr(count)
        temp2 = sectionhandle(temp, tempstr2)
        If temp2 = -1 Then
            createsection temp, tempstr2
            temp2 = sectionhandle(temp, tempstr2)
        End If
        temp = temp2
    End If
...
Public Sub SetMultiKey(Section As String, key As String, Value As String, Optional delimeter As String = vbNewLine)
    ...
    If temp > 0 Then
        ...
        For temp2 = 0 To UBound(tempstr)
            If keyexists(temp, key, temp2 + 1) = True Then
                setkey temp, key, tempstr(temp2), temp2 + 1
            Else
